...
# try:
#     from YOffleNet.mish_cuda import MishCuda as Mish
# except:
...
class mish_cuda(nn.Module):  # https://github.com/digantamisra98/Mish
    def forward(self, x):
        return x * torch.nn.functional.softplus(x).tanh()
...
class Separable_Conv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        ...
        self.conv2 = nn.Conv2d(c1, 1, 1, s, autopad(1, p), groups=1, bias=False)
...
class BottleneckCSP(nn.Module):
    ...
    def forward(self, x):
        ...
        y2 = self.cv2(x)
        #print('CSP size='self.cv4(self.act(self.bn(torch.cat((y1, y2), dim=1)))).shape)
...
class SPPCSP(nn.Module):
    ...
    def forward(self, x):
        ...
        y2 = self.cv2(x)
        # print('============spp',self.cv7(self.act(self.bn(torch.cat((y1, y2), dim=1)))).shape)
...
class Conv3(nn.Module):
    ...
    def __init__(self, inp,oup,stride):
        ...
        self.Conv3 = nn.Conv2d(inp, oup, 3, stride, 1, bias=False)

    def forward(self, x):
       # print('conv3======',self.Conv3(x).shape)
        return self.Conv3(x)
...
class LastConv(nn.Module):
    ...
    def __init__(self, inp , oup ):
        ...
        #nn.Conv2d(oup, 117, 1, 1, 0, bias=False)
        )
    def forward(self, x):
        #print('Lastconv========',self.Last(x).shape)
        return self.Last(x)
############################################################################
...
class InvertedResidual(nn.Module):
    ...
    def forward(self, x):
        ...
        # print('inverted=======',channel_shuffle(out, 2).shape)
